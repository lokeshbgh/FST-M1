...
public class Activity2 
{
	...
	public void addNewUserFromFile() throws IOException {
		...
		response.then().body("message", equalTo("12341234"));
	}
	...
	public void getUserInfo() {
		...
		Response response = given().
			baseUri("https://petstore.swagger.io/v2/user"). // Set base URI
			header("Content-Type", "application/json"). // Set headers
			pathParam("username", "TestUser"). // Pass request body from file
		...
		// Assertion
		response.then().body("id", equalTo(12341234));
		response.then().body("username", equalTo("TestUser"));
		response.then().body("firstName", equalTo("TestUser"));
		response.then().body("lastName", equalTo("TestUser"));
		response.then().body("email", equalTo("TestUser@mail.com"));
	...
	public void deleteUser() throws IOException {
		Response response = given().
			baseUri("https://petstore.swagger.io/v2/user"). // Set base URI
			header("Content-Type", "application/json"). // Set headers
			pathParam("username", "TestUser"). // Add path parameter
